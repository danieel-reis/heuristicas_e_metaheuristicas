 ...
 * Como compilar: g++ tp5_daniel_reis.cpp -o tp5
 * Como executar: ./tp5
 */
...
Solution VND(Graph graph, Solution initialSolution, int lmax) {
    ...
    while (l < lmax) {  // Realiza lmax iterações
        Solution bestNeighboor = neighboor2OPTSWAP(graph, initialSolution);        // Gera dois vizinhos permutando e pega o melhor vizinho
        if (bestNeighboor.getCost() < initialSolution.getCost()) {                 // Se o melhor dos vizinhos tem custo menor do que a melhor solução
            initialSolution = bestNeighboor;                                       // Atualiza a melhor solução
            l = 1;
        }
        else
            l++;
...
//     for(auto e : initialSolution.getPath()) { std::cout << e << ","; } std::cout << " ---------- Cost: " << initialSolution.getCost() << std::endl;
//    std::cout << initialSolution.getCost() << ",";
...
void ILS(Graph graph, int lmax, int perm1, int perm2) {
    Solution solution = heuristicNearestNeighbor(graph, false);             // Gera a solução inicial
//     for(auto e : initialSolution.getPath()) { std::cout << e << ","; } std::cout << " ---------- Cost: " << initialSolution.getCost() << std::endl;
    ...
    for (int i = 0; i < perm1; i++) {
        ...
        if (i < perm1 - 1) {
            for (int j = 0; j < perm2; j++) {
                news = neighboor2OPTSWAP(graph, news);        // Gera dois vizinhos permutando e pega o melhor vizinho
            }
        }
...
int main() {
    ...
    do {
        ...
        std::cout << "| 1 - ILS - EXECUTAR UMA INSTANCIA                                    |" << std::endl;
        std::cout << "| 2 - ILS - EXECUTAR INSTANCIAS DE UM DIRETORIO                       |" << std::endl;
